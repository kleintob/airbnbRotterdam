...
plot(density(residualsFullModel), main = "Density of Residuals", xlab = "Residuals", ylab = "Density")

# Superimpose a normal distribution
x_vals <- seq(min(residualsFullModel), max(residualsFullModel), length = 100)
...
df <- 100  # Example degrees of freedom, replace with actual df from your model
...
ggplot(data, aes(x = x, y = y)) +
  ...
  labs(title = "t-distribution with two-sided rejection region", 
  ...
  annotate("text", x = -critical_value - 0.2, y = 0.05, label = paste("critical value:", round(-critical_value, 2)), color = "black") +
  annotate("text", x = critical_value + 0.2, y = 0.05, label = paste("critical value:", round(critical_value, 2)), color = "black") +
  annotate("text", x = test_stat + 0.2, y = 0.1, label = paste("test statistic:", round(test_stat, 2)), color = "red") +  # test statistic label in red
  theme_minimal() +
...
ggplot(data, aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = test_stat, linetype = "solid", color = "red", linewidth = 1) +  # test statistic line in red
  annotate("text", x = critical_value_one_sided + 0.2, y = 0.05, label = paste("critical value:", round(critical_value_one_sided, 2)), color = "black") +
  annotate("text", x = test_stat + 0.2, y = 0.1, label = paste("test statistic:", round(test_stat, 2)), color = "red") +  # test statistic label in red
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))  # center the title
```
...
ggplot(data, aes(x = x, y = y)) +
  ...
  annotate("text", x = lower_bound + 0.05, y = 0.05, label = paste("lower bound:", format(round(lower_bound, 3), nsmall = 3)), color = "black") +
  annotate("text", x = upper_bound - 0.05, y = 0.05, label = paste("upper bound:", format(round(upper_bound, 3), nsmall = 3)), color = "black") +
  annotate("text", x = estimate + 0.05, y = 0.1, label = paste("estimate:", format(round(estimate, 3), nsmall = 3)), color = "red") +  # estimate label in red
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))  # center the title
```
...
ggplot(data, aes(x = x, y = y)) +
  ...
  annotate("text", x = critical_value_f + 0.1, y = 0.1, label = paste("critical value:", format(round(critical_value_f, 3), nsmall = 3)), color = "black") +
  annotate("text", x = F_stat + 0.1, y = 0.2, label = paste("test statistic:", format(round(F_stat, 3), nsmall = 3)), color = "red") +  # test statistic label in red
  annotate("text", x = F_stat + 0.1, y = 0.15, label = paste("p-value:", format(round(p_value_f, 3), nsmall = 3)), color = "blue") +  # p-value label in blue
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))  # center the title
```
